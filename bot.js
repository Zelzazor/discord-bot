...
const functions = {
    ...
    addPlaylist: async (message, serverQueue) => {
        ...
        const permissions = voiceChannel.permissionsFor(message.client.user);
        if (!permissions.has("CONNECT") || !permissions.has("SPEAK")) {
            console.log("¡No tengo permisos! Dile al administrador que me dé permisos.");
          return message.channel.send(
            "¡No tengo permisos! Dile al administrador que me dé permisos."
          );
        }
            ...
            if (ytpl.validateID(args[1])) {
                ...
                console.log("Enlace de playlist inválido. Revise el enlace e intente de nuevo.");
            ...
            if (!serverQueue) {
                ...
                try {
                    ...
                    queueContruct.connection = connection;
                    functions.playSong(message.guild, queueContruct.songs[0]);
                    console.log(`¡La playlist ${metadata.title} Ha sido añadida a la cola!`);
        ...
        const dispatcher = serverQueue.connection
            ...
            stream.on("error", error => {
                try{
                    ...
                    stream.destroy();
                    console.log("Ha ocurrido un error inesperado con la canción que se intentó reproducir.");
                    serverQueue.textChannel.send("Ha ocurrido un error inesperado con la canción que se intentó reproducir.")
                    console.log(error);
                }
            });
    ...
    removeSpecificSong: (message, serverQueue) => {
        let args = message.content.trim().split(/ +/g);
        if(args.length != 2){
            console.log("¡Cantidad de argumentos incorrecto! Uso: /remove <número> - Ejemplo: /remove 1")
        ...
        if(args[1] < 1 || args[1] > serverQueue.songs.length){
            console.log("/remove Índice inválido. Revise la cantidad de canciones presentes en la cola e intente con un índice de canción válido");
